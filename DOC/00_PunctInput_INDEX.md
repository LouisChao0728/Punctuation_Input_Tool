...
**對應版本**：v1.4.1（2026-07-11，Aphy 分支——45 鍵改 1 列 10 項、安裝前先解除既有安裝；master 對應 v1.3）
...
| 版本 | v1.4.1（Aphy 分支，manifest assembly version 1.4.1.0），2026-07-11 |
...
符號按鍵（FR-003；Aphy 版 10 欄配置——v1.4.1 老闆裁決「1 列 10 項」，列數依符號數推導；成對鍵一鍵成對輸入）。Aphy 分支共 45 鍵（10 欄 5 列，末列 5 鍵），依序：
...
| 路徑 | 職責 | 備考 |
|------|------|------|
| `src\Program.cs` | C# WinForms 單檔全部邏輯：視窗建置、DPI 縮放、全域熱鍵、符號送出三路路由（DD-4／DD-9，含剪貼簿快照與還原）、系統匣、除錯日誌（FR-013） | 781 行（2026-07-11 v1.4.1 實查）；語言層級 C# 5（NFR-05） |
| `src\app.manifest` | DPI 感知宣告（`dpiAware=true`，NFR-03）+ Common Controls v6 相依宣告 | assembly version 1.4.1.0（Aphy 分支） |
...
| 文件 | 用途 | 版本 |
|------|------|------|
| `00_PunctInput_INDEX.md` | 本檔，專案索引 | v1.4.1（2026-07-11） |
| `01_PunctInput_PRD.md` | 產品需求文件（含設計決策紀錄 DD-1 至 DD-9、風險 R1 至 R6） | v1.3（2026-07-11，本輪未涉及） |
| `02_PunctInput_SPEC_v1.0.md` | 專案規格書（實作基準；檔名依慣例維持 v1.0，內容就地更新） | v1.4.1（2026-07-11） |
| `03_PunctInput_SRS_v1.0.md` | 軟體需求規格書（FR-001 至 FR-014、NFR-01 至 NFR-05；檔名依慣例維持 v1.0，內容就地更新） | v1.4.1（2026-07-11） |
| `04_PunctInput_MODIFY_LOG.md` | 異動摘要紀錄（最新版在上） | v1.4.1（2026-07-11） |
