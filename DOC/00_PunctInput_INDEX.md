...
**對應版本**：v1.3.1（2026-07-12，剪貼簿快照依 DD-10 限縮文字類白名單，修正延遲渲染擁有者致 UI 凍結）
...
| 版本 | v1.3.1（manifest assembly version 1.3.1.0），2026-07-12 |
...
7 個符號按鍵依序（4 欄 2 列，FR-003；v1.2 起括號成組一鍵成對輸入）：
...
| 路徑 | 職責 | 備考 |
|------|------|------|
| `src\Program.cs` | C# WinForms 單檔全部邏輯：視窗建置、DPI 縮放、全域熱鍵、符號送出三路路由（DD-4／DD-9，含剪貼簿快照——文字類白名單 DD-10——與還原）、系統匣、除錯日誌（FR-013） | 780 行（2026-07-12 v1.3.1 實查）；語言層級 C# 5（NFR-05） |
| `src\app.manifest` | DPI 感知宣告（`dpiAware=true`，NFR-03）+ Common Controls v6 相依宣告 | assembly version 1.3.1.0 |
...
| `dist\PunctInput.exe` | 建置產出（可執行檔） | 17,408 bytes（2026-07-12 v1.3.1 實查值） |
| `DOC\` | 本專案文件目錄 | 詳見第四節 |
...
| 文件 | 用途 | 版本 |
|------|------|------|
| `00_PunctInput_INDEX.md` | 本檔，專案索引 | v1.3.1（2026-07-12） |
| `01_PunctInput_PRD.md` | 產品需求文件（含設計決策紀錄 DD-1 至 DD-10、風險 R1 至 R6） | v1.3.1（2026-07-12） |
| `02_PunctInput_SPEC_v1.0.md` | 專案規格書（實作基準；檔名依慣例維持 v1.0，內容就地更新） | v1.3.1（2026-07-12） |
| `03_PunctInput_SRS_v1.0.md` | 軟體需求規格書（FR-001 至 FR-014、NFR-01 至 NFR-05；檔名依慣例維持 v1.0，內容就地更新） | v1.3.1（2026-07-12） |
| `04_PunctInput_MODIFY_LOG.md` | 異動摘要紀錄（最新版在上） | v1.3.1（2026-07-12） |
...
| 建置產出 | `dist\PunctInput.exe`（17,408 bytes，2026-07-12 v1.3.1 實查值） |
