...
| `ClientSize` | 寬 `Scale(GRID_COLS*78+16)`＝874、高 `Scale(48+gridRows*62+20)`；v1.4.2 起 `GRID_COLS`＝11、列數 `gridRows`＝`SymbolRows.Length`＝5（列結構定義），高 378，實測含框 880 × 407 px | 版面計算 |
...
1. 佈局（v1.4.2 老闆裁決「1 列 11 項」＋獨立列）：`GRID_COLS`＝11 為列容量上限，實際配置由 `SymbolRows` 列結構陣列逐列定義（不足 11 鍵之列於右側留空，不建按鈕）；master 維持 v1.2 裁決之 4 欄流水排列。Aphy 45 鍵 = 5 列（11／11／2／11／10）。
...
Aphy 分支擴充 38 鍵（v1.4.2 依老闆指示重排：✿ ❀ 移至 ⛤ 之後；圈號與點號各自獨立成列）：

| 列 | 按鍵 | 送出內容（碼位） | 備考 |
|----|------|------------------|------|
| 第 1 列（11 鍵） | 基礎 7 鍵 + 〔〕 ﹝﹞ ← → | 〔〕＝U+3014 U+3015、﹝﹞＝U+FE5D U+FE5E（成對）；←＝U+2190、→＝U+2192 | 基礎 7 鍵碼位見上表 |
| 第 2 列（11 鍵） | ➤ ❥ ♥ ♡ ► ◄ ⇒ ✔ ✓ ☑ ⛤ | U+27A4、U+2765、U+2665、U+2661、U+25BA、U+25C4、U+21D2、U+2714、U+2713、U+2611、U+26E4 | ♥ 為 Boss_Prompt 清單第 5 項 Facebook 表情圖檔之實體字元 |
| 第 3 列（2 鍵） | ✿ ❀ | U+273F、U+2740 | 花卉（v1.4.2 自末位移至 ⛤ 之後） |
| 第 4 列（11 鍵） | ⓪ ① 至 ⑩ | U+24EA、U+2460 至 U+2469（連續） | 圈號數字（獨立列） |
| 第 5 列（10 鍵） | ⒈ 至 ⒑ | U+2488 至 U+2491（連續） | 點號數字（獨立列） |

### 5.2 按鈕配置

1. 配置由 `SymbolRows[row][col]` 列結構直接決定（v1.4.2；master 為 4 欄流水排列 `i % 4`／`i / 4`），每列由左至右、列間由上至下。
2. Aphy 分支 45 鍵 = 5 列（11／11／2／11／10）；第 3 列右側 9 欄與第 5 列右側 1 欄留空。
