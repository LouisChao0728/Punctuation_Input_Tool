...
**文件版本**：v1.3.1（2026-07-12；檔名依 AssetM 慣例維持 v1.0，就地更新並於文末註記沿革）
...
4. 版本：v1.3.1（`app.manifest` assemblyIdentity version `1.3.1.0`），日期 2026-07-12。
...
└── dist/
    └── PunctInput.exe         master 建置產出（17,408 bytes，2026-07-12 v1.3.1）；Aphy 分支產出 PunctInput_Aphy.exe，dist 雙檔並存
```
...
4. 剪貼簿路徑欄位：`route=ClipboardPaste`、`text=U+XXXX`、`focus`、`class`；還原成功記 `clipboard restored`，設定或還原失敗記 `clipboard set failed: ...`／`clipboard restore failed: ...`。v1.3.1 起新增兩個中途觀測點（消除 route 行與還原行之間的不可觀測區間）：快照完成記 `clipboard snapshot done backup=text|none`、貼上鍵注入後記 `ctrl+v injected`。
5. 略過送字時記：`SendSymbolToTarget skip: no usable foreground window`。

### 7.6 剪貼簿中轉（DD-9／DD-10，`SendViaClipboardPaste`）

1. 快照（v1.3.1 依 DD-10 改文字類白名單）：送字前以 `SnapshotClipboard()` 備份使用者剪貼簿，僅限固定白名單 `SnapshotTextFormats`（`UnicodeText`／`Text`／`Rtf`／`Html`）——先以 `GetDataPresent`（僅查詢格式存在、不觸發渲染）確認存在，才 `GetData` 取值；非文字格式一律不備份。緣由：v1.3 原快照對全部格式逐一 `GetData`，遇延遲渲染（delayed rendering）擁有者（Illustrator 2020 實機案例）會同步等待擁有者現場渲染（單格式最長約 30 秒、多格式累積），UI 執行緒凍結、Ctrl + V 永遠送不出去。還原尚未執行（`_restorePending`）時不重拍快照，確保連續點擊期間保住原始內容。
...
| 產出 | `dist\PunctInput.exe`（17,408 bytes，2026-07-12 v1.3.1）；Aphy 分支為 `dist\PunctInput_Aphy.exe`（v1.4 系列），雙檔並存互不覆蓋 |
...
- **R6（剪貼簿中轉副作用，DD-9／DD-10）**：送字時剪貼簿內容被替換約 0.5 秒後盡力還原；v1.3.1 起僅備份文字類格式（DD-10 白名單），非文字格式（圖形、物件等）不還原——使用者原複製之非文字內容須重新複製；目標應用延遲處理貼上時有貼到還原後內容之競態（§7.6 第 5 點）。
...
| V14 | v1.3.1 快照白名單三案端對端實測（V11 至 V13 為 Aphy 分支驗證編號） | T1 新版＋文字剪貼簿：WPF 非 EDIT 目標收到「」，日誌完整鏈 route → `snapshot done backup=text` → `ctrl+v injected` → `clipboard restored`（+499 ms），剪貼簿還原原內容（PASS）；T2 新版＋HeavyOwner 模擬延遲渲染擁有者（兩個非文字延遲格式、WM_RENDERFORMAT 沉睡 20 秒）：快照 3 ms 內跳過（backup=none）、符號正常送達、還原正常、面板保持回應（PASS）；T3 舊版 v1.3＋同一擁有者：重現凍結簽名——僅 route 一行、面板無回應、目標零輸出（根因因果閉環）（2026-07-12） |
...
| DD-10 | 剪貼簿快照限縮為文字類格式白名單（v1.3.1；`UnicodeText`／`Text`／`Rtf`／`Html`，先 `GetDataPresent` 再 `GetData`，非文字格式不備份；解決延遲渲染擁有者致 UI 執行緒凍結問題，候選「背景執行緒＋逾時」「重量內容跳過快照」落選） | 老闆裁決（2026-07-12 三案選 A；Aphy 使用者 Illustrator 實機案例，凍結經使用者確認、本機 HeavyOwner 模擬重現） |

## 十三、 操作方式

1. 安裝（FR-014，建議方式）：於檔案總管雙擊 `scripts\install.bat`（以 `%~dp0` 自我定位，與開啟位置無關），或先 `cd` 至專案根目錄後執行 `powershell -ExecutionPolicy Bypass -File scripts\install.ps1`（相對路徑，於其他起始目錄執行會回報「-File 引數不存在」）——先偵測既有安裝（安裝目錄或任一捷徑存在），有則先執行 `uninstall.ps1` 完整解除舊版（2026-07-11 老闆指示，master 與分支共同規則）；再部署 exe 至 `%LOCALAPPDATA%\Programs\PunctInput\`、建立開始功能表捷徑與開機自啟捷徑（Startup 資料夾）並啟動；`-NoStartup` 略過自啟捷徑、`-NoLaunch` 安裝後不啟動；`dist\PunctInput.exe` 缺檔時自動先執行 `build.ps1`。
2. 解除安裝：雙擊 `scripts\uninstall.bat`，或於專案根目錄執行 `powershell -ExecutionPolicy Bypass -File scripts\uninstall.ps1`——停止程序、移除兩處捷徑與安裝目錄，不動原始碼與 `dist\`。
...
*本文件為 PunctInput 實作與改動基準，與現行 `src\Program.cs`、`scripts\build.ps1`、`src\app.manifest` 逐一對應；後續異動須同步更新本文件相關章節與版本沿革註記。沿革：v1.0（2026-07-11）首版建立；v1.1（2026-07-11）呼叫快捷鍵依 DD-8 改為 Ctrl + Alt + /，V7 結案、R1 緩解、R2 老闆採認、新增 V8；v1.2（2026-07-11）括號成組一鍵成對輸入（7 鍵）、4 欄 2 列配置、快捷鍵新增數字鍵盤 VK_DIVIDE 鍵位、新增 V9；v1.3（2026-07-11）非 EDIT 目標送字依 DD-9 改剪貼簿中轉自動貼上（新增 §7.6、R6、V10，R4 改寫）；v1.3.1（2026-07-12）剪貼簿快照依 DD-10 限縮文字類白名單並新增兩個除錯觀測點（§7.5、§7.6 第 1 點、R6 改寫，新增 DD-10、V14）。*
