...
3. 呼叫方式：全域快捷鍵 Ctrl + / 切換視窗顯示／隱藏（FR-001）；程序常駐系統匣，關閉視窗僅隱藏，結束程序須由系統匣選單「結束」（DD-3、FR-010）。
4. 版本：v1.0（`app.manifest` assemblyIdentity version `1.0.0.0`），日期 2026-07-11。
...
|------|--------|--------|--------|------|----|
| Ctrl + / | `HOTKEY_ID_TOGGLE`＝1 | `MOD_CONTROL`(0x0002) \| `MOD_NOREPEAT`(0x4000) | `VK_OEM_2`(0xBF) | 顯示／隱藏切換 | FR-001 |
| Esc | `HOTKEY_ID_ESC`＝2 | `MOD_NOREPEAT`(0x4000) | `VK_ESCAPE`(0x1B) | 隱藏視窗 | FR-002 |

### 6.2 生命週期

1. Ctrl + / 於 `OnHandleCreated` 註冊、`OnHandleDestroyed` 反註冊（整個程序存活期間有效）。
...
| 情境 | 行為 | 對應 |
|------|------|------|
| Ctrl + / 註冊失敗（被佔用） | `OnHandleCreated` 顯示警告訊息框，程序續行，仍可由系統匣操作 | FR-012 |
| 重複啟動（已有實例） | `Mutex` `createdNew=false`，彈出「標點符號輸入工具已在執行中，請以 Ctrl + / 呼叫。」提示後結束 | FR-011 |
...
- **R1（全域熱鍵衝突）**：Ctrl + / 為全域熱鍵，與應用程式內快捷鍵衝突（例：VS Code 切換註解），全域熱鍵優先。
- **R2（裸 Esc 攔截代價）**：視窗顯示期間裸 Esc 被全域攔截，前景應用程式的 Esc（IME 組字取消、關閉選單等）會被吃掉一次並隱藏面板，面板隱藏後即恢復。對抗審查 3/3 確認之設計代價，已裁決保留（Boss_Prompt 明定 Esc 關閉、不搶焦點約束下無 `KeyPreview` 替代路徑），本文件揭露供老闆知悉。
- **R3（UIPI）**：無法輸入至以系統管理員權限執行的視窗（WM_CHAR 與 SendInput 皆被阻擋，靜默失敗，除錯日誌可查）。
- **R4（TSF 相容性假設）**：SendInput 路徑對 TSF 應用（Chromium、Word）在 IME 開啟時的相容性為「假設」，待老闆實機驗證（以 `PUNCTINPUT_DEBUG=1` 查路由）。
...
| V5 | Ctrl + / 呼叫 | `keybd_event` 注入組合鍵，視窗實測重現（PASS） |
| V6 | 對抗審查 | 3 視角（interop／spec／robust）Opus 審查共 16 項發現，3 人反駁表決確認 5 項（已修正或裁決文件化）、否決 11 項 |
| V7 | 待驗證（老闆實機） | 實際滑鼠點擊送字至常用應用程式（Obsidian、瀏覽器、Word、LINE）之路由正確性 |
...
2. 呼叫：Ctrl + / 顯示／隱藏；Esc 或視窗關閉鈕隱藏；系統匣右鍵「結束」退出程序。
3. 重建：`powershell -ExecutionPolicy Bypass -File scripts\build.ps1`。

---

*本文件 v1.0（2026-07-11）為 PunctInput 實作與改動基準，與現行 `src\Program.cs`、`scripts\build.ps1`、`src\app.manifest` 逐一對應；後續異動須同步更新本文件相關章節與版本沿革註記。*
